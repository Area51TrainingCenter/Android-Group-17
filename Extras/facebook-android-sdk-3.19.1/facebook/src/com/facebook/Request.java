 ...
 * href="https://developers.facebook.com/docs/reference/api/">Graph API</a>. The
 * Request class provides functionality relating to serializing and
 ...
 * 
 * The particular service endpoint that a request targets is determined by a
 * graph path (see the {@link #setGraphPath(String) setGraphPath} method).
 * 
...
public class Request {
	...
	public static Request newMyUploadPhotoRequest(Session session,
			Bitmap image, String caption, String description, Callback callback) {
	 ...
	 * with `true` for this user. <b>You must call this method from a background
	 * thread for it to work properly.</b>
	 * 
	 * @param session
	 ...
	 * it, or c) the app has previously called
	 * {@link Settings#setLimitEventAndDataUsage(android.content.Context, boolean)}
	 * ;} with `true` for this user. <b>You must call this method from a
	 * background thread for it to work properly.</b>
	 * 
	 * @param session
	 *            the Session to issue the Request on, or null; if non-null, the
	 *            session must be in an opened state. If there is no logged-in
	...
	private void addCommonParameters() {
		...
		} else if (!skipClientToken
				&& !this.parameters.containsKey(ACCESS_TOKEN_PARAM)) {
	...
	final String getUrlForSingleRequest() {
		...
		String baseUrl = String.format("%s/%s", graphBaseUrlBase,
			...
			if (passByValue) {
				...
				if (jsonObject.has("id")) {
					...
					processGraphObjectProperty(key,
						...
						.has(NativeProtocol.OPEN_GRAPH_CREATE_OBJECT_KEY)) {
					processGraphObjectProperty(key, jsonObject.toString(),
							serializer, passByValue);
				}
			}
